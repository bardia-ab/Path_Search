proc gen_bit {Proj_Dir Src_Dir Store_path CR folder_name N_Parallel} {
	...
	set bitstream_file "$Store_path/Bitstreams/$CR/$folder_name\.bit"
	set DCP_file "$Store_path/DCPs/$CR/$folder_name\.dcp"
	#set logfile "$Proj_Dir/$ProjName\.runs/impl_1/runme.log"
	#set logfile_dest "$Store_path/Logs/$CR/$folder_name\.log"
...
if {$even_odd_opt == {None}} {
	for {set i $TC_start_idx} {$i < $n_TCs} {incr i} {
		set folder_name "TC$i"
		catch {gen_bit $Proj_Dir $Src_Dir $Store_path $CR $folder_name $N_Parallel}
		set logfile_dest "$Store_path/Logs/$CR/$folder_name\.log"
		catch {file copy $logfile $logfile_dest}
	}
} elseif {$even_odd_opt == {custom}} {
    #set $Store_path "$Store_path/Split_Bitstreams"
